...
export default function AccountPage() {
  ...
  const handleSetDefault = async (addressId: string) => {
    try {
      await axiosInstance.put(
        ...
        { address_id: Number(addressId) }, // 👈 send payload
        { headers: { Authorization: `Bearer ${token}` } }
      );
    ...
    if (window.confirm("Are you sure you want to delete this address?")) {
      try {
        await axiosInstance.delete(`/users/profiles/address/${userId}`, {
          data: { address_id: Number(addressId) }, // 👈 body must be under `data`
          headers: { Authorization: `Bearer ${token}` },
        });
  ...
  const handlePasswordUpdateClick = () => {
    const confirmed = window.confirm(
      "Are you sure you want to update your password? You will be redirected to a secure password update page."
    );
    if (confirmed) {
